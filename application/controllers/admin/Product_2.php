...
class Product extends CI_Controller {
    ...
    function index() {
        ...
        $total_records = $this->product_model->get_product(array('count' => TRUE));
        $pagination_data = array('base_url' => ADMIN_SITE_URL . 'product/index', 'total_rows' => $total_records, 'per_page' => 10, 'uri_segment' => 4);
        $pagination_config = geneate_pagination($pagination_data);

        $this->pagination->initialize($pagination_config);
        $this->data['page'] = ($this->uri->segment(4)) ? $this->uri->segment(4) : 0;
        $query_data = array('limit' => 10, 'offset' => $this->data['page']);
        $this->data['product_data'] = $this->product_model->get_product($query_data);
        $this->data['pagination'] = $this->pagination->create_links();
        $this->layout->view('admin/product/index', $this->data);
    }
    ...
    public function delete($id) {
        if ($id != '') {
            ...
            $this->product_model->update_product($upd_data);
        }
        redirect('admin/product');
    }
