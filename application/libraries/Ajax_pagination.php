...
class Ajax_pagination {
    ...
    var $base_class = ''; // The class we are linking to
    var $base_method = ''; // The method we are linking to
    ...
    function create_links() {
        ...
        } else if ($CI->uri->segment('3') != 0) {
            $this->cur_page = $CI->uri->segment('3');
        } else if ($CI->uri->segment('5') != 0) {
            $this->cur_page = $CI->uri->segment('5');
        }
        ...
        if ($this->show_count) {
            if ($CI->uri->segment($this->uri_segment) != 0) {
                ...
                $curr_offset = $CI->uri->segment('3');
            } else {
                $curr_offset = $CI->uri->segment('5');
            }
        ...
        if ($this->cur_page != 1) {
            ...
            $output .= $this->prev_tag_open
                    . $this->getAJAXlink($i, $this->prev_link,$style='style="float:left;"')
                    . $this->prev_tag_close;
        }
    ...
    function getAJAXlink($count, $text,$style='') {
        if ($this->div == '')
            return '<a '.$style.' href="' . $this->anchor_class . ' ' . $this->base_url . $count . '">' . $text . '</a>';
        if ($this->additional_param == '')
            $this->additional_param = "{'t' : 't'}";

        $base_class = $this->base_class;
        $base_method = $this->base_method;
        //return "<a ref=\"#\" " . $this->anchor_class . " onclick=\"ajax_pagination('$base_class','$base_method','$count')\">" . $text .'</a>';
        return "<a $style ref=\"#\" " . $this->anchor_class . " onclick=\"get_question_on_pagination('$count')\">" . $text . '</a>';
        /* return "<a ref=\"#\"
          " . $this->anchor_class . "
